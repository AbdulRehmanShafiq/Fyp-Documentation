...
- **Closed learning loop (Intelligence Roadmap Phase 1, shipped 2026-07-01).** A tenant's confirmed/corrected transactions become labeled signals. `learnedResolution.service` learns a normalized description → accounts mapping (via `learningKey.helper` + the existing `EntityMemory` store, kind `nl_description_accounts`) on confirm/auto-post, and recalls it in the NL parser **ahead of** the fuzzy matcher — a learned mapping beats a guess, but never overrides an explicit user choice (the preview is still editable). `aiCalibration.service` derives measured acceptance/reversal rates from the Phase-0 ledger and an **effective** auto-post threshold that is **conservative-only** — it can raise the bar when a tenant reverses auto-posts but NEVER lowers it below the static 0.98 floor (correctness > convenience). Read surface: `GET /api/v1/ai-decisions/stats`.
- **Continuous Close & STP (Intelligence Roadmap Phase 3, shipped 2026-07-02).** `stpScorecard.service.getScorecard` derives windowed automation rates (posting/matching/reconciliation/categorization) read-only from existing records — no stored metric can drift from the ledger. `closeReadiness.service.getReadiness` scores month-end readiness over the real engines; a failing check source reports "not verified" and blocks, never a false pass. Endpoints under `/autonomy/`.
- **Proactive AI CFO (Intelligence Roadmap Phase 4, shipped 2026-07-02).** `advisor.service.getRecommendations` gathers fault-isolated signals (13-week cash forecast, AR aging, health, STP) and `advisor.helper.buildRecommendations` renders ranked plain-language recommendations grounded by construction. Every non-empty run is recorded in the AI Decision Ledger (kind `recommend`). Advice framing is "what your numbers say" — never investment advice.
- **Explainability everywhere (Intelligence Roadmap Phase 2, shipped 2026-07-01).** Every decision renders a grounded plain-language "why" via `aiExplain.helper.buildExplanation` — deterministic templating over the stored record only, so a hallucinated rationale is impossible (faithful by construction, same philosophy as `narrative.service`). `GET /api/v1/ai-decisions/:id/explain` returns it; `POST /api/v1/ai-decisions/:id/outcome` is the one-click accept/correct/reverse surface, and a **correction feeds the Phase 1 learning loop**. Copy stays plain per the product-copy rule (no debit/credit jargon as primary text).

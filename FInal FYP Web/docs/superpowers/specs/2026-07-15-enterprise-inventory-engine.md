...
| **1. Stock movement ledger** | StockMovement model + writes at every stock touch (receive, sell, reverse, adjust); ledger endpoint reads movements; `inventoryIntegrity.computeDrift`; rebuild script (movements → item projection) | none (observability substrate) | Stock-ledger endpoint switched off JE heuristics; drift check reads 0 on seed data |
| **2. Adjustments + counts** | Adjustment doc (increase/decrease/write-off/revalue + reason codes), physical count session (snapshot → count → variance post), NRV write-down w/ reversal cap | recipes §2.3; every adjustment = movement + JE pair in one txn | UI: Inventory page "Adjust stock" + count flow; 6495 wired |
| **3. Returns integration** | CreditNote lines gain restock (qty + condition); VendorCredit lines gain return-to-vendor; both post movement + JE with the doc | sale-return / purchase-return recipes | Returns round-trip: sell→return→resell keeps drift 0 |
